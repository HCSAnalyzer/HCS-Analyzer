        ...
        private void StandardNormalization()
        {
            ...
            for (int PlateIdx = 0; PlateIdx < NumberOfPlates; PlateIdx++)
            {
               ...
               // GlobalInfo.ConsoleWriteLine("Plate: " + CurrentPlateToProcess.Name);
                ...
                {
                    if (CompleteScreening.ListDescriptors[Desc].IsConnectedToDatabase == true)
                    {
                        GlobalInfo.ConsoleWriteLine("Cell by cell normalization not implemented yet. " + CompleteScreening.ListDescriptors[Desc].GetName() + " skipped");
                        continue;
                    }
                    ...
                    for (int row = 0; row < CompleteScreening.Rows; row++)
                        for (int col = 0; col < CompleteScreening.Columns; col++)
                        {
                            ...
                            if (TempWell.GetClassIdx() == comboBoxNormalizationNegativeCtrl.SelectedIndex) Neg.Add(TempWell.ListDescriptors[Desc].GetValue());
                        }
                    ...
                    for (int row = 0; row < CompleteScreening.Rows; row++)
                        for (int col = 0; col < CompleteScreening.Columns; col++)
                        {
                           ...
                           // for (int i = 0; i < TempWell.ListDescriptors[Desc].GetAssociatedType().GetBinNumber(); i++)
                            {
                                double Value = TempWell.ListDescriptors[Desc].GetValue() - CurrentMean;
                         ...
                         //   for (int i = 0; i < TempWell.ListDescriptors[Desc].GetAssociatedType().GetBinNumber(); i++)
                            {
                                ...
                                TempWell.ListDescriptors[Desc].SetHistoValues(Val * 100);
                ...
                for (int Desc = 0; Desc < NumDesc; Desc++)
                {
                    ...
                    for (int row = 0; row < CompleteScreening.Rows; row++)
                        for (int col = 0; col < CompleteScreening.Columns; col++)
                        {
                            TempWell = CurrentPlateToProcess.GetWell(col, row, true);
                            if (TempWell == null) continue;
                            if (TempWell.GetClassIdx() == comboBoxNormalizationPositiveCtrl.SelectedIndex) Pos.Add(TempWell.ListDescriptors[Desc].GetValue());
                            if (TempWell.GetClassIdx() == comboBoxNormalizationNegativeCtrl.SelectedIndex) Neg.Add(TempWell.ListDescriptors[Desc].GetValue());
                        }
                    ...
                    for (int row = 0; row < CompleteScreening.Rows; row++)
                        for (int col = 0; col < CompleteScreening.Columns; col++)
                        {
                            ...
                            //for (int i = 0; i < TempWell.ListDescriptors[Desc].GetAssociatedType().GetBinNumber(); i++)
                            {
                                double CurrValue = TempWell.ListDescriptors[Desc].GetValue();
                                TempWell.ListDescriptors[Desc].SetHistoValues((CurrValue - CurrentMeanNeg) / Denominator);
