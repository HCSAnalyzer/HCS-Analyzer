...
namespace LibPlateAnalysis
{
    ...
    public class cDBConnection
    {
        ...
        public void DisplayTable(cWell Well)
        {
            ...
            DataTable dt = new DataTable();
            dt.Load(value);
            cDisplayScatter2D WindowForTable = new cDisplayScatter2D(dt, Well.AssociatedPlate.ParentScreening.GlobalInfo);
        ...
        public int AddWellToDataTable(cWell Well, DataTable DataTableToAddedTo, cGlobalInfo GlobalInfo)
        {
            ...
            int CurrentClass = Well.GetClassIdx();

            if (DataTableToAddedTo.Columns.Count == 0)
            {
                foreach (string TmpName in Names)
                {
                    int DescIdx = GlobalInfo.CurrentScreen.ListDescriptors.GetDescriptorIndex(TmpName);
                    if ((DescIdx != -1) && (GlobalInfo.CurrentScreen.ListDescriptors[DescIdx].IsActive()))
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                {
                    int DescIdx = GlobalInfo.CurrentScreen.ListDescriptors.GetDescriptorIndex(Names[IdxCol]);
                    if ((DescIdx != -1) && (GlobalInfo.CurrentScreen.ListDescriptors[DescIdx].IsActive()))
                    {
        ...
        public cExtendedTable GetWellValues(cWell TmpWell, cDescriptorsType DescType)
        {
            List<cDescriptorsType> LCDT = new List<cDescriptorsType>();
            LCDT.Add(DescType);

            cExtendedTable ToReturn = new cExtendedTable(this.GetWellValues(TmpWell.SQLTableName, LCDT));

            cListSingleBiologicalObjects ListClassesPhenotypes =
                this.GetWellBiologicalPhenotypes(/*this.GetWellValues(TmpWell.SQLTableName, DescType),*/ TmpWell);
        ...
        public cExtendedTable GetWellValues(cWell TmpWell, List<cDescriptorsType> ListDescType)
        {
        ...
        public cExtendedTable GetWellValues(cWell TmpWell, List<cDescriptorsType> ListDescType, List<cCellularPhenotype> PhenotypesToBeSelected)
        {
            cExtendedTable ToReturn = new cExtendedTable();
        ...
        public cListSingleBiologicalObjects GetWellBiologicalPhenotypes(cWell TmpWell)
        {
            ...
            if (Pos != -1)
            {
                while (value.Read())
                {
                    ToReturn.Add(new cSingleBiologicalObject(TmpWell.Parent.GlobalInfo.ListCellularPhenotypes[(int)value.GetFloat(Pos)], TmpWell));
                }
            }
            else
            {
                while (value.Read())
                {
                    ToReturn.Add(new cSingleBiologicalObject(TmpWell.Parent.GlobalInfo.ListCellularPhenotypes[0], TmpWell));
                }
        ...
        public void CreateNewColumn(string ColumnName, double DefaultValue)
        {
    ...
    public class cPlate : cGeneralComponent
    {
        ...
        public cListWell ListActiveWells = new cListWell(null);
        ...
        public Instances CreateInstancesWithClasses(List<bool> ListClassSelected)
        {
            ...
            weka.core.FastVector attVals = new FastVector();
            foreach (var item in ParentScreening.GlobalInfo.ListWellClasses)
            {
            ...
            foreach (cWell CurrentWell in this.ListActiveWells)
            {
                if (!ListClassSelected[CurrentWell.GetClassIdx()]) continue;
                double[] vals = new double[data1.numAttributes()];
                ...
                vals[columnNo] = CurrentWell.GetClassIdx();
                data1.add(new DenseInstance(1.0, vals));
                IdxWell++;
            }
            data1.setClassIndex((data1.numAttributes() - 1));
        ...
        public int[] UpdateNumberOfClass()
        {
            ...
            for (int j = 0; j < ParentScreening.Rows; j++)
                for (int i = 0; i < ParentScreening.Columns; i++)
                {
                    ...
                    ListNumObjectPerClasse[TempWell.GetClassIdx() + 1]++;
                }
        ...
        public cInfoClass GetNumberOfClassesBut(int NeutralClass)
        {
            ...
            InfoClass.CorrespondanceTable = new int[ParentScreening.GlobalInfo.GetNumberofDefinedWellClass()];
        ...
        public int GetNumberOfActiveWellsButClass(int NeutralClass)
        {
            ...
            for (int row = 0; row < ParentScreening.Rows; row++)
                for (int col = 0; col < ParentScreening.Columns; col++)
                    if ((GetWell(col, row, true) != null) && (GetWell(col, row, true).GetClassIdx() != NeutralClass)) NumberOfActive++;
        ...
        public void Display3Dplate(int IdxDescriptor, cPoint3D MinimumPosition)
        {
            ...
            {
                if ((TmpWell == null) || (TmpWell.GetClassIdx() == -1)) continue;
                ...
                if (ParentScreening.GlobalInfo.OptionsWindow.checkBoxDisplayWellInformation.Checked)
                {
                    ...
                    if (ParentScreening.GlobalInfo.OptionsWindow.radioButtonWellInfoDescValue.Checked)
                        ToDisp = TmpWell.ListDescriptors[ParentScreening.ListDescriptors.CurrentSelectedDescriptorIdx].GetValue().ToString("N3");
        ...
        public cWell GetWell(int Col, int Row, bool OnlyIfSelected)
        {
            ...
            if ((OnlyIfSelected) && (ListWell[Col, Row].GetClassIdx() == -1))
                return null;
            ...
            for (int j = 0; j < ParentScreening.Rows; j++)
                for (int i = 0; i < ParentScreening.Columns; i++)
                {
                    ...
                    if (currentWell == null)
                        ...
                        Table[i, ParentScreening.Rows - j - 1] = currentWell.GetAverageValuesList(false)[Desc];
                }
        ...
        public double[,] GetWellClassesTable()
        {
            ...
            for (int j = 0; j < ParentScreening.Rows; j++)
                for (int i = 0; i < ParentScreening.Columns; i++)
                {
                    ...
                    if (currentWell == null) continue;
                    Table[i, ParentScreening.Rows - j - 1] = currentWell.GetClassIdx();
                }
        ...
        {
            int PosPlate = this.ParentScreening.GlobalInfo.WindowHCSAnalyzer.toolStripcomboBoxPlateList.FindStringExact(this.Name);
            this.ParentScreening.GlobalInfo.WindowHCSAnalyzer.toolStripcomboBoxPlateList.SelectedIndex = PosPlate;
            this.ParentScreening.CurrentDisplayPlateIdx = PosPlate;

            this.ParentScreening.GetCurrentDisplayPlate().DisplayDistribution(this.ParentScreening.ListDescriptors.CurrentSelectedDescriptorIdx, false);
        }
        ...
        public void CopyValuestoClipBoard()
        {
            ...
            sb.Append(String.Format("{0}", ParentScreening.Columns - 1));
            sb.AppendLine();
            ...
            for (int j = 0; j < ParentScreening.Rows; j++)
            {
                ...
                for (int i = 0; i < ParentScreening.Columns - 1; i++)
                {
                    ...
                    if (CurrentWell == null)
                        ...
                        sb.Append(String.Format("{0}\t", CurrentWell.ListDescriptors[ParentScreening.ListDescriptors.CurrentSelectedDescriptorIdx].GetValue()));
                }
                ...
                if (CurrentWellFinal == null)
                    ...
                    sb.Append(String.Format("{0}\t", CurrentWellFinal.ListDescriptors[ParentScreening.ListDescriptors.CurrentSelectedDescriptorIdx].GetValue()));
            ...
            for (int j = 0; j < ParentScreening.Rows; j++)
            {
                ...
                for (int i = 0; i < ParentScreening.Columns - 1; i++)
                {
                    ...
                    if (CurrentWell == null)
                        ...
                        sb.Append(String.Format("{0}\t", CurrentWell.GetClassIdx()));
                }
                ...
                if (CurrentWellFinal == null)
                    ...
                    sb.Append(String.Format("{0}\t", CurrentWellFinal.GetClassIdx()));
        ...
        private void ToolStripMenuItem_Clustering(object sender, EventArgs e)
        {
            ...
            ParentScreening.GlobalInfo.WindowHCSAnalyzer.PerformScreeningClustering(ListPlate, true);
        }
