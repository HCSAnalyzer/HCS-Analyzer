        ...
        public void RefreshDisplay(bool IsOnlyColors)
        {
           // System.Drawing.Graphics formGraphics = this.panelForPlates.CreateGraphics();
            ...
            for (int i = 0; i < PanelList.Count; i++)
            {
                ...
                //if (i > 0) formGraphics.DrawLine(new Pen(Color.Tomato), 
                //    new Point(PanelList[i - 1].Location.X+PanelList[i - 1].Width/2, PanelList[i - 1].Location.Y+PanelList[i - 1].Height/2),
                //    new Point(PanelList[i].Location.X+PanelList[i].Width/2, PanelList[i].Location.Y+PanelList[i].Height/2));
            }
            ...
            else
            {
                for (int j = 0; j < ValuesMatrix.Length; j++)
                    for (int i = 0; i < ValuesMatrix[0].Length; i++)
                    {
                        ...
                        SolidBrush Brush = new SolidBrush(TmpWell.GetColor());
    ...
    public partial class FormToDisplayPlate : cPanelForDisplayArray
    {
        ...
        private void FormToDisplayPlate_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                contextMenuStrip.Items.AddRange(new ToolStripItem[] { ToolStripMenuItem_RemovePlate });
