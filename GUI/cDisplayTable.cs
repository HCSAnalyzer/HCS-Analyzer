            ...
            if (IsMINE)
            {
                ...
                for (int Row = 0; Row < Values.Count; Row++)
                {
                    ...
                    for (int idxString = 0; idxString < Values[Row].Length; idxString++)
                    {
                        ...
                        {
                            int ConvertedValue = (int)((GlobalInfo.LUTs.LUT_GREEN_TO_RED[0].Length - 1) * Math.Abs(double.Parse(CurrentString)));

                            Color Coul = Color.FromArgb(GlobalInfo.LUTs.LUT_GREEN_TO_RED[0][ConvertedValue], GlobalInfo.LUTs.LUT_GREEN_TO_RED[1][ConvertedValue], GlobalInfo.LUTs.LUT_GREEN_TO_RED[2][ConvertedValue]);
